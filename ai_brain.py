...
# ذاكرة المحادثات - كل user_id له تاريخ محادثة
conversation_memory = {}
MAX_HISTORY = 10  # آخر 10 رسائل لكل عضو
...
async def get_ai_response(user_question: str, user_id: int = 0) -> tuple:
    ...
    # نتحقق إذا العضو له تاريخ محادثة (يعني هو يكمل الحديث)
    has_history = user_id in conversation_memory and len(conversation_memory[user_id]) > 0

    # إذا له تاريخ، نتخطى الكاش وقاعدة المعرفة ونذهب مباشرة لـ Claude
    if not has_history:
        ...
        if knowledge:
            ...
            # نحفظ في الذاكرة أيضاً حتى يستطيع Claude متابعة الحوار
